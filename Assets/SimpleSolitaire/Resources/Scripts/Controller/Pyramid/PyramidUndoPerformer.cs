...
{
    public class PyramidUndoPerformer : UndoPerformer
    {
        protected override string LastGameKey => "PyramidLastGame";
        public override UndoData StatesData
        {
            get => _statesData;
            set => _statesData = (PyramidUndoData)value;
        }
        private PyramidUndoData _statesData = new PyramidUndoData();
        ...
        public override void Initialize()
        {
            ...
            AotHelper.EnsureList<PyramidUndoStates>();
            AotHelper.EnsureList<PyramidCardRecord>();
            AotHelper.EnsureList<PyramidDeckRecord>();
        }
        ...
        public override void AddUndoState(Deck[] allDeckArray, bool isTemp = false)
        {
            ...
            StatesData.States.Add(new PyramidUndoStates(allDeckArray, isTemp));
        }
        ...
        public override void LoadGame()
        {
            if (PlayerPrefs.HasKey(LastGameKey))
            {
                ...
                StatesData = DeserializeData<PyramidUndoData>(lastGameData);
        ...
        protected override void UndoProcess()
        {
            ...
            {
                PyramidDeckRecord deckRecord = _statesData.States[statesCount - 1].DecksRecord[i] as PyramidDeckRecord;
                ...
                if (deck.HasCards)
                {
                    for (int j = 0; j < deckRecord.CardsRecord.Count; j++)
                    {
                        PyramidCard card = deck.CardsArray[j] as PyramidCard;
                        PyramidCardRecord cardRecord = deckRecord.CardsRecord[j] as PyramidCardRecord;
                        ...
                        CardPositionInfo info = Logic.LayoutContainer.GetInfoById(cardRecord.InfoId);
                        card.Info = info;
#if UNITY_EDITOR
            ...
            Logic.PyramidDeck.UpdateCardsPosition(false);
            Logic.PyramidTrashDeck.UpdateCardsPosition(false);
        }
        ...
        public override bool IsHasGame()
        {
            ...
            {
                string lastGameData = PlayerPrefs.GetString(LastGameKey);
                UndoData data = DeserializeData<PyramidUndoData>(lastGameData);
