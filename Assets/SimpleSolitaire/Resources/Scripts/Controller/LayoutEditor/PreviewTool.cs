...
namespace SimpleSolitaire.Controller
{
    public class PreviewTool : MonoBehaviour
    {
        ...
        public void MakePreview(List<LayoutCard> cards, string path, Action<Texture2D> onPreviewCreated = null)
        {
            StartCoroutine(TakeSnapShotAndSave(cards, path, onPreviewCreated));
        }

        private IEnumerator TakeSnapShotAndSave(List<LayoutCard> cards, string path, Action<Texture2D> onPreviewCreated = null)
        {
